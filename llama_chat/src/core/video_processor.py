...
class VideoProcessor:
    def __init__(self):
        # Use CPU only for better stability and lower resource usage
        self.device = "cpu"
        logger.info(f"Using device: {self.device}")
        
        # Initialize Whisper with minimal settings
        ...
        # Optimized frame sampling settings - reduced sampling rate
        self.frame_sampling = {
            'short_form': {'interval': 5, 'early_frames': 3.0},  # Reduced from 2 and 5
            'long_form': {'interval': 15, 'early_frames': 10.0}  # Increased from 10 and 15
        }
        
        # Reduced batch processing settings
        self.batch_size = 2  # Reduced from 4
        self.max_workers = 1  # Reduced from 2
        
        # Initialize vision model - simple version
        try:
    ...
    async def analyze_video(self, video_path: str) -> Dict:
        """Analyze video content with optimized processing"""
        try:
            ...
            # Determine video type and sampling strategy
            ...
            # Process audio first (most time-consuming)
            logger.info("Processing audio...")
            audio_analysis = await self._process_audio(video_path)
            
            # Process frames in parallel batches with reduced load
    ...
    async def _process_frames_parallel(self, cap, total_frames: int, interval: int, early_frames: float) -> List[Dict]:
        """Process video frames in parallel batches with reduced load"""
        frames = []
        frame_batches = []
        current_batch = []
        
        # Calculate frames to process - reduced sampling
        frames_to_process = []
        ...
        async def process_batch(batch):
            ...
            for frame_idx in batch:
                ...
                if ret:
                    ...
                    frame = cv2.resize(frame, (320, 240))  # Reduced from original size
    ...
    def _analyze_frame(self, frame: np.ndarray) -> str:
        """Analyze a single frame with minimal processing"""
        try:
            # Convert to grayscale for faster processing
            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            
            # Use a much smaller image for analysis
            small = cv2.resize(gray, (80, 60))  # Reduced from 160x120
            
            # Basic scene analysis with minimal processing
    ...
    async def _process_audio(self, video_path: str) -> Dict:
        """Process audio with minimal settings"""
        try:
